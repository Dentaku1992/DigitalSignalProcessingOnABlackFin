...
void process()
{
	...
	{
		for( j=LINEOFFSET; j<LINELENGTH; j+=4)
		{
			//AVF odd
			...
			if(Cb > MIN_CB && Cr > MIN_CR)
			{
				...
				{
					//Adapt odd AND even line at once
					...
					//Adapt pixels to the right as well
					video_frame[i*LINELENGTH+j+2] = 0xEB28;
					video_frame[i*LINELENGTH+j+1+2] = 0xEB98;
					video_frame[(i+AVOFFSET)*LINELENGTH+j+2] = 0xEB28;
					video_frame[(i+AVOFFSET)*LINELENGTH+j+1+2] = 0xEB98;
				}
